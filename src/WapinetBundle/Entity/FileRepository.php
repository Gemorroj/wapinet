...
class FileRepository extends EntityRepository
{
    ...
    public function getStatistic($maxUsers = 10)
    {
        ...
        $countFiles = $em->createQuery('SELECT COUNT(f.id) FROM WapinetBundle\Entity\File f');
        $sizeFiles = $em->createQuery('SELECT SUM(f.fileSize) FROM WapinetBundle\Entity\File f');
        $countViews = $em->createQuery('SELECT SUM(f.countViews) FROM WapinetBundle\Entity\File f');
    ...
    public function countHidden()
    {
        return $this->getEntityManager()->createQuery(
            'SELECT COUNT(f.id) FROM WapinetBundle\Entity\File f WHERE f.password IS NULL AND f.hidden = 1'
    ...
    {
        return $this->getEntityManager()->createQuery(
            'SELECT COUNT(f.id) FROM WapinetBundle\Entity\File f WHERE f.password IS NULL AND f.hidden = 0'
        )->getSingleScalarResult();
    }
